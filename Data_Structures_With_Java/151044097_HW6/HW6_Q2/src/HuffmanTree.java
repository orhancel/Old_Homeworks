...
public class HuffmanTree
  ...
  protected void traversal(BinaryTree<HuffData> rootNode ,String yol,Character key  ){



    if(rootNode!=null){
       if(rootNode.getData().symbol==key){
          path=yol;
          return;
       }

      String temp=yol;
      traversal(rootNode.getLeftSubtree(),temp+="0",key);
      temp=yol;
      traversal(rootNode.getRightSubtree(),temp+="1",key);
    }

  }

  public String encode(String lineToCode){
    StringBuilder result = new StringBuilder();
    BinaryTree < HuffData > currentTree = huffTree;
    for (int i=0;i<lineToCode.length();i++){
      String yol="";
      traversal(currentTree,yol,lineToCode.charAt(i));
      result.append(path);
      System.out.println(path+" ");////Print line.It's okay to remove.
    }
    return result.toString();
