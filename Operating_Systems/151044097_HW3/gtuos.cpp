...
uint64_t GTUOS::handleCall(const CPU8080 & cpu){
	...
	// Get a random integer.
	...
	else if(cpu.state->a == (uint8_t)12){


		cout << "Call: FileCreate   Thread  id:"<< current_thread->thread_id << endl;

		Dosya yeni;

		yeni.creation_time=time(0);
    ...
    else if(cpu.state->a == (uint8_t)14){


        cout << "Call: FileOpen   Thread  id:"<< current_thread->thread_id << endl;
        char name[256];
        char ch='x';
        for(int i=0; ch != '\0'; ++i){
            ch=(char)cpu.memory->at(address+i);
        ...
        for(int i=0;i<Dosyalar.size();i++){
            if(  strcmp(Dosyalar.at(i).name,name)==0 ){
                Dosyalar.at(i).is_open= true;
                cpu.state->b=Dosyalar.at(i).file_id;
                return 40;
            }
        }
    ...
    else if(cpu.state->a == (uint8_t)13){


        cout << "Call: FileClose   Thread  id:"<< current_thread->thread_id << endl;

        for(int i=0;i<Dosyalar.size();i++){
            if(Dosyalar.at(i).file_id==cpu.state->b){
                Dosyalar.at(i).is_open= false;
                cpu.state->b=1;
                return 40;
            }
        }
    ...
    else if(cpu.state->a == (uint8_t)15){

        cout << "Call: FileRead   Thread  id:"<< current_thread->thread_id << endl;

        Dosya oku;

        for(int i=0;i<Dosyalar.size();i++){

            if( Dosyalar.at(i).file_id == (int)cpu.state->e ){
        ...
        for(int i=oku.cursor;i< cpu.state->d ;i++){

            if(oku.content.size()>i)
                cpu.memory->at(address+i) = oku.content[i];
            else{
        ...
        cpu.memory->at(address+int(cpu.state->d))= (uint8_t)'\0';

        oku.cursor+=cpu.state->d;
    ...
    else if(cpu.state->a == (uint8_t)16){

        cout << "Call: FileWrite   Thread  id:"<< current_thread->thread_id << endl;
        ...
        for(int i=0; i<cpu.state->d ; ++i){
            oku.content.push_back((char)cpu.memory->at(address+i));
        }

        oku.file_size+=cpu.state->d;
    ...
    else if(cpu.state->a == (uint8_t)17){

        cout << "Call: FileSeek   Thread  id:"<< current_thread->thread_id << endl;
        ...
        for(int i=0;i<Dosyalar.size();i++){
            if( Dosyalar.at(i).file_id == (int)cpu.state->d ){
                oku=Dosyalar.at(i);
            }
        }

        if((int)cpu.memory->at(address) > oku.file_size ){
            cpu.state->b=0;
            return 40;
        }


        oku.cursor = cpu.memory->at(address);
